...
ignore_directories = ['.git', 'assets', 'examples']
preprocess_files = [
	'utilities/color.js',
	'utilities/float.js',
	'views/view.js',
...
def process_file(full_file_path, output_text):
	print("Compiling " + full_file_path)
	with open(full_file_path, 'r') as openFile:
		output_text += openFile.read()
	output_text += "\n"
	return output_text

for preprocess_file in preprocess_files:
	output_text = process_file(preprocess_file, output_text)

rootdir = os.path.dirname(os.path.realpath(__file__))
for item in os.listdir(rootdir):
	if os.path.isdir(item):
		if item in ignore_directories:
			print("Will not compile: " + item + " because it is in ignore_directories in compile.py")
			continue
		else:
			directory = item
			...
			for root, subFolders, files in os.walk(directory):
				for file in files:
					full_path = root + "/" + file
					if full_path not in preprocess_files:
						output_text = process_file(full_path, output_text)
					else:
						print("Will not compile: " + item + " because already compiled it in preprocessing")
