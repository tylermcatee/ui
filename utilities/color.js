...
function interpolatedColor(min, max, percent) {
	...
	var lerpBlue = Math.floor(flerp(minRGB['b'], maxRGB['b'], percent));

	var lerpColor = _rgbToHex(lerpRed, lerpGreen, lerpBlue);
	
	return lerpColor;
}

function _hexForCSSDefault(color) {
    if (typeof __HTMLColorsToHex[color.toLowerCase()] != 'undefined')
        return __HTMLColorsToHex[color.toLowerCase()];
    return null;
}
